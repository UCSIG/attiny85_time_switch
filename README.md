...
The aim of this project is to switch a load on and off in regular time intervals. The device itselfs consumes only very little sleep current (~19.6uA on average).  
...
The code resides unter the folder `src` and `include`. Platformio is chosen as the build environment. The best way to program the ATtiny85 is to use Platformio and VS Code as the IDE.  
